# 简介
这是一个脚手架，包含「eslint」代码规范的脚手架，同时我也会在下面写出我个人认为较好的组件规范。使用的配置和插件是目前较为常用的，配置结构清晰，保证你只要看得明白「webpack」官方文档，就绝对能看明白的配置文件，同时也会在下方列出所有的可配置项。

这还是一个包含「Server-Side-Render」的脚手架，因为「服务端渲染」和一般的「客户端渲染」在运行方式上存在天然的不同，所以在写法上会存在一些差异，主要是数据的预处理上，我们需要「状态管理」来把所有的数据提前存储起来，在下面的说明中我会详细介绍原因和写法。
...
### 代码规范
代码规范工具使用的是「eslint」，代码规则来源的是据说是最严格的「airbnb」，当然最严格的并不一定是最好的，所以我也根据我个人的习惯修改了部分的规则。如果你在使用中对其中的规则有自己看法、有意见，也可以提给我，我们一起来修改「代码规则」，让它用起来更得心应手。
